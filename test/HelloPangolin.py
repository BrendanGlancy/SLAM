...
def main():
    ...
    # Define Projection and initial ModewlView matrix
    scam = pangolin.OpenGlRenderState(
            pangolin.ProjectionMatrix(640, 480, 420, 420, 320, 240, 0.2, 100),
            pangolin.ModelViewLookAt(-2,2,-2,0,0,0, pangolin.AxisDirection.AxisY))
    handler = pangoline.Handler3D(scam)
    ...
    while not pangolin.ShouldQuit():
        ...
        colors[:, 1] = 1 - point[;, 0] / 10.
        colors[:, 2] = 1 - points[:, 1] / 10.
        colors[:, 0] = 1 - points[:, 2] / 10

        gl.glPointSize(2)
        gl.glColor3f(1.0, 0.0, 0.0)
        # access numpy array directly(without copying data), array should be continguous.
